...
import com.zykj.xishuashua.view.customBorderDrawable;
	...
	public static void xiaoMiShortCut(Context context,Class<?> clazz, String num)
	  {
		...
		try {
			...
			field.set(miuiNotification, num);// 设置信息数-->这种发送必须是miui 6才行
		} catch (Exception e) {
		    e.printStackTrace();
		    //miui 6之前的版本
	...
	public static void sonyShortCut(Context context, String num)
	  {
		...
		localIntent.putExtra("com.sonyericsson.home.intent.extra.badge.ACTIVITY_NAME", "com.zykj.xishuashua.Welcome");//启动页activityName
		localIntent.putExtra("com.sonyericsson.home.intent.extra.badge.MESSAGE", num);//数字
		localIntent.putExtra("com.sonyericsson.home.intent.extra.badge.PACKAGE_NAME", "com.zykj.xishuashua");//包名context.getPackageName()
		context.sendBroadcast(localIntent);
	  }
	...
	public static void samsungShortCut(Context context, String num)
	 {
		...
		localIntent.putExtra("badge_count", StringUtil.isEmpty(num)?0:Integer.valueOf(num));//数字
		localIntent.putExtra("badge_count_package_name", "com.zykj.xishuashua");//包名
		localIntent.putExtra("badge_count_class_name","com.zykj.xishuashua.Welcome"); //启动页
		context.sendBroadcast(localIntent);
	 }
	...
	public static void addNumShortCut(Context context,Class<?> clazz,boolean isShowNum, String num, boolean isStroke)
	  {
		if (TextUtils.isEmpty(num)) {
			...
			int numInt = Integer.valueOf(num);
			num = String.valueOf(Math.max(0, Math.min(numInt, 99)));
		}
