...
public class StoreGiftAdapter extends CommonAdapter<Gift> {
	...
	public StoreGiftAdapter(Context context, int resource, List<Gift> datas) {
	...
	public void convert(final ViewHolder holder, Gift gift) {
		...
		try {
			...
			String collectTime = DateUtil.longToString(Long.parseLong(gift.getCollect_lastsawtime()+"000"), "yyyy-MM-dd");
			holder.setText(R.id.gift_name, gift.getGoods_name())
				...
				.setText(R.id.gift_type, "1".equals(gift.getGrade_id())?"个人红包":"2".equals(gift.getGrade_id())?"商家红包":"app红包")
				.setText(R.id.gift_label, gift.getGoods_serial())
				.setText(R.id.store_date, collectTime+" 收藏")
			...
			if(holder.getPosition() == 0){
				holder.setVisibility(R.id.store_date, true);
			}else{
				long time = Long.parseLong(mDatas.get(holder.getPosition()-1).getCollect_lastsawtime()+"000");
				String converTime = DateUtil.longToString(time, "yyyy-MM-dd");
				if(!collectTime.equals(converTime)){
					holder.setVisibility(R.id.store_date, true);
				}
			}
