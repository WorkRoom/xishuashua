...
public class GiftAdapter extends CommonAdapter<Gift> {
	...
	public void convert(final ViewHolder holder, Gift gift) {
		...
		String status = "news".equals(gift.getStore_name())?"2":marketprice>0?"1":"0";//新闻news 即时1 永久0
		try {
			String sellTime = DateUtil.longToString(Long.parseLong(gift.getGoods_selltime()+"000"), "yyyy-MM-dd");
			holder.setText(R.id.gift_name, gift.getGoods_name())//红包名称
				.setText(R.id.gift_content, gift.getGoods_jingle())//红包简介
				...
				.setVisibility(R.id.gift_time, !"0".equals(status))
